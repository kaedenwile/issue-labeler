...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            console.log(`sync-labels = ${core.getInput('sync-labels', { required: false })}`);
            ...
            if (addLabel.length > 0) {
                ...
                yield addLabels(client, issue_number, addLabel);
            }
            if (syncLabels) {
                console.log("SYNCING LABELS"); // TODO
                yield Promise.all(removeLabelItems.map(function (label) {
                    console.log(`Removing label ${label} from issue #${issue_number}`);
                    return removeLabel(client, issue_number, label);
                }));
            }
            else {
                console.log("NOT SYNCING LABELS"); // TODO
            }
        }
        catch (error) {
            core.error(error);
